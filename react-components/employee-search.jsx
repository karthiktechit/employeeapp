...
var EmployeeSearch = React.createClass({
	getInitialState: function(){
		return {
			searchResult: [],
			employees: [{"id":1,"gender":"male ","title":"Mr. ","firstName":"Tom","surname":"Roberts","dob":"21/04/1986","age":29,"salary":"59,783.00","takeHome":"41,999.84","incomeTax":"13,316.20","nationalInsurance":"4,466.96"},
...
{"id":30,"gender":"male ","title":"Mr. ","firstName":"Morgan","surname":"Taylor","dob":"16/09/1993","age":22,"salary":"63,299.00","takeHome":"44,039.12","incomeTax":"14,722.60","nationalInsurance":"4,537.28"}]
	...
	onfirstNameChange: function(event){
		...
		for (var i = 0; i < employees.length; i++) {
			  firstNames.push(employees[i].firstName);
			}
		this.search({type:'firstName', keyval : event.target.value})
	},
	onSurNameChange: function(event){
		var surnames = [];
		var employees = this.state.employees;
		for (var i = 0; i < employees.length; i++) {
			  surnames.push(employees[i].surname);
			}
		this.search({type:'surname', keyval : event.target.value})
	},
	search : function(searchDtl){
		var employees = this.state.employees;
		var matches = [];
		var searchVal = searchDtl.keyval.toLowerCase();
			for (var i = 0; i < employees.length; i++) {
				if(searchVal.indexOf(employees[i][searchDtl.type].toLowerCase()) > -1){
					  matches.push(employees[i]);
				}
			}
			this.setState({searchResult: matches});
	},
	render: function(){
		if(this.state.matches!= null && this.state.matches.length > 0){
		}
		return(
			<div className='addnote'>
				<input type='text' className='firstName' placeholder='First Name' onChange={this.onfirstNameChange}/>
				<input type='text' placeholder='Surname' onChange={this.onSurNameChange}/>
